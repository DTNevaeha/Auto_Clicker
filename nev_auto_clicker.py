...
class AutoClickerGUI:
    def __init__(self, root, clicker):
        ...
        self.button_label = tk.Label(
            frame,
            text=f"Current Click: {'Left' if self.clicker.is_left_click else 'Right'}",
        )
        self.button_label.grid(row=0, column=0, pady=(10, 0))

        self.click_button = tk.Button(
            frame, text="Toggle to Right Click", command=self.toggle_click_button
        )
        self.click_button.grid(row=0, column=1, pady=(10, 20))

        self.interval_label = tk.Label(
            frame, text=f"Click Interval: {self.clicker.interval} seconds"
        )
        self.interval_label.grid(row=1, column=0)

        self.interval_button = tk.Button(
            frame, text="Set Click Interval", command=self.set_interval
        )
        self.interval_button.grid(row=1, column=1)
        
        
        self.interval_label_2 = tk.Label(frame, text=f"Enter New Speed Below")
        self.interval_label_2.grid(row=3, column=0)
        ...
        self.toggle_button = tk.Button(
            frame, text="Start Clicking", command=self.toggle_clicking
        )
        self.toggle_button.grid(row=4, column=1, pady=(10, 0))
        ...
        self.counter_button = tk.Button(
            frame, text="Increase Counter", command=self.increase_counter
        )
        self.counter_button.grid(row=6, column=1)

        self.hotkey_label = tk.Label(frame, text=f"Change Keybind to Toggle Clicking")
        ...
        self.update_hotkey_button = tk.Button(
            frame, text="Update Hotkey", command=self.update_hotkey
        )
        self.update_hotkey_button.grid(row=8, column=1)

        self.hotkey_label_2 = tk.Label(
            frame, text=f"Change Keybind to Toggle Left/Right"
        )
        ...
        self.update_hotkey_button_2 = tk.Button(
            frame, text="Update Hotkey", command=self.update_hotkey_2
        )
        ...
        root.bind(
            self.current_hotkey, self.on_hotkey_press
        )  # Bind click to on_hotkey_press
        root.bind(
            self.current_hotkey_2, self.on_hotkey_press_2
        )  # Bind toggle_click to on_hotkey_press_2
    ...
    def set_interval(self):
        try:
            ...
            self.interval_label.config(
                text=f"Current Click Interval: {self.clicker.interval} seconds"
            )
        except ValueError:
            pass
    ...
    def toggle_click_button(self):
        ...
        self.click_button.config(
            text=f"Toggle to {'Right' if self.clicker.is_left_click else 'Left'} Click"
        )
...
root = tk.Tk()
root.geometry("400x400")  # Set the default size of the window to 400x400 pixels
clicker = AutoClicker()
